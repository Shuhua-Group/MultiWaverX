...
void print_help()
{
	...
	std::cout << " -i/--input     <string>    Input of the autosomal ancestral tracks [required]" \
			<< std::endl;
	...
	std::cout << " -t/--thread	[integer]   Number of thread [optional, default 1]" \
			<< std::endl;
	std::cout << " -C/--chr       <string>    Chromosome type, \"A\" for Autosome, \"X\" for X chromosome " \
			"[optional, default A]" << std::endl;
	std::cout << " -x/--inputX    <string>    Input of the X chromosome ancestral tracks [optional, required if -C = X]" \
			<< std::endl;
...
void argparse(int argc, char **argv)
{
	...
	while((ch = getopt_long(argc, argv,"i:o:l:a:e:p:m:w:shc:b:M:t:C:x:g:", long_options, &option_index)) != -1)
	{
		switch(ch)
		{
			case 'i' :
			...
			{
				SoftPar.chr = optarg;
				if (SoftPar.chr != "A" && SoftPar.chr != "X")
				{
					except e;
					e << "Error: The chromosome type is incorrect!";
					throw e;
				}
				break;
			}
